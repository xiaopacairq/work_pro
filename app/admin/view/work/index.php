        ...
        <div class="layui-form-item">
            ...
            <div class="layui-inline">
                <button class="layui-btn layui-btn-primary layui-border-black"
                    onclick="add('{$class.class_id}')">布置作业</button>
                <button
                    class="layui-btn layui-btn-primary layui-border-blue <?= count($work) == 0 ? "layui-btn-disabled" : "" ?>"
                    onclick=" get_zip('{$class.class_id}')" <?= count($work) == 0 ? "disabled" : "" ?>>作业导出</button>
            </div>
        </div>
    </div>
    <div class="layui-row layui-col-space15">
        ...
        <div class="layui-col-md4">
            <div class="layui-panel"
                style="color:<?= (($recent_date > $work['work_last_time']) || ($work['status'] == 1)) ? "#ccc" : "" ?> ;">
                ...
                <div class="panel-foot">
                    ...
                    <button type="button" class="layui-btn layui-btn-sm  layui-btn-warm"
                        onclick='edit("{$class.class_id}","{$work.id}","{$work.work_id}")'>
...
function get_zip(class_id) {
    layer.confirm('确定要下载吗？', {
        ...
        window.location.href = '/teacher/class_work_get_zip?class_id=' + class_id;
    });
}
...
function add(class_id) {
    layer.open({
        ...
        shadeClose: false,
        anim: 5,
        content: '/teacher/class_work_add?class_id=' + class_id,
        offset: 't',
        shade: 0.3,
        shadeClose: true
    });
...
function edit(class_id, id, work_id) {
    layer.open({
        ...
        shadeClose: false,
        anim: 5,
        content: '/teacher/class_work_edit?class_id=' + class_id + '&id=' + id + ' &work_id= ' + work_id,
        offset: 't',
        shade: 0.3,
        maxmin: true,
        shadeClose: true
