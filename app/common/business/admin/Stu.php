...
class Stu
{
    ...
    public function delStu($class_id, $stu_no, $is_clear_all)
    {
        if ($is_clear_all) { //清空所有数据
            // 清空 stu \ is_work \ works \ score 数据表
            ...
            $errCode = $this->createFile($class_id);
            if ($errCode !=  config('status.success')) {
                if ($errCode == config('status.error')) {
                    return config('status.error');
                } else {
    ...
    {
        $admin = $this->adminModel->findAdminByUname($data['admin']);
        $class_id = $data['class_id'];
        $class_name = $data['class_name'];
        $stu_no = $data['stu_no'];

        if ($is_all == 0) { //单独发送密码
            $stu = $this->stuModel->findStu($class_id, $stu_no);
     ...
     * 删除班级的信息储存表
     */
    private function delFile($class_id)
    {
     ...
     * 创建班级的信息储存表
     */
    private function createFile($class_id)
    {
